...
import type { LoginResponse } from '@/api/apiclients/ZincApiClient'

const STORAGE_KEY = 'zinc_auth'
...
export function useAuth() {
  ...
  const user = computed(() => authData.value ? {
    ...
    displayName: `${authData.value.firstName} ${authData.value.lastName}`.trim(),
    username: authData.value.username,
    role: authData.value.role
  } : null)
